...
public class GlobalExceptionHandler {
    ...
    /**
     * Gère les exceptions liées au service de localité
     *
     * @param e       L'exception interceptée
     * @param request La requête Web
     * @return Une réponse HTTP avec le message d'erreur et le code d'état 503 Service Unavailable
     */
    @ExceptionHandler(LocalityServiceException.class)
    @ResponseStatus(HttpStatus.SERVICE_UNAVAILABLE)
    public ResponseEntity<ApiError> handleLocalityServiceException(
            LocalityServiceException e, WebRequest request) {
        ApiError error = new ApiError(
                "Erreur de communication avec le service de localité",
                e.getMessage(),
                request.getDescription(false));
